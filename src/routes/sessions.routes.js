...
router.get('/googlecallback', passport.authenticate('google', { failureRedirect: '/login' }), async (req, res) => {
    try {
        ...
        res.redirect('https://front-end-coder-d3u5a0j03-lorenzoalbornozs-projects.vercel.app');
    } catch (error) {
        res.status(500).json({
    ...
    try {
        ...
        const mailOptions = {
            ...
            html: `
                ...
                <a href="https://front-end-coder-d3u5a0j03-lorenzoalbornozs-projects.vercel.app/reset_password/${user._id}/${encodeURIComponent(token)}">
        ...
        jwt.verify(token, config.PRIVATE_KEY, async (err, decoded) => {
            if (err) {
                if (err.name === 'TokenExpiredError') {
                    ...
                    return res.redirect('https://front-end-coder-d3u5a0j03-lorenzoalbornozs-projects.vercel.app/api/login');
                } else {
